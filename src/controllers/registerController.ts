...
export default class RegisterController {
    ...
    registerUser = async (
        call: { request: {name: string, email: string; mobile: string, password: string, userImage: string ,otp: string} },
        callback: (error: any, response: any) => void
      ) => {
        const { name, email, password, mobile, userImage, otp} = call.request;
        try {
          const response = await registerUseCase.registerUser(name, email, password, mobile, userImage, otp);
          callback(null, response);
        } catch (error) {
